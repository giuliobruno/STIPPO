...
export function MemoryCard({ memory }: { memory: MemoryCardData }) {
  ...
  const tags = usefulTags(memory.tags, { sourceUrl: memory.sourceUrl, limit: 3 });

  return (
    <div className="vm-card overflow-hidden">
      <Link
        href={`/app/memories/${memory.id}`}
        className="group block transition duration-200"
      >
        ...
        <div className="space-y-2 p-3.5 sm:p-4">
          ...
          {isLink && memory.sourceUrl ? (
            <p className="truncate text-sm text-[var(--accent)]">
              {displayUrl(memory.sourceUrl)}
            </p>
          ) : memory.description ? (
      ...
      {isLink && memory.sourceUrl ? (
        <div className="flex items-center gap-2 border-t border-[var(--line)] px-3.5 py-2.5 sm:px-4">
          <a
            href={memory.sourceUrl}
            target="_blank"
            rel="noopener noreferrer"
            className="inline-flex flex-1 items-center justify-center gap-2 rounded-[var(--radius-sm)] bg-[var(--accent)] px-3 py-2 text-sm font-medium text-white transition hover:opacity-90"
          >
          ...
          <Link
            href={`/app/memories/${memory.id}`}
            className="inline-flex shrink-0 items-center justify-center rounded-[var(--radius-sm)] border border-[var(--line)] px-3 py-2 text-sm font-medium text-[var(--ink)] transition hover:border-[var(--accent)]/40 hover:text-[var(--accent)]"
          >
            Edit
          </Link>
        </div>
